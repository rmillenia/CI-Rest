...
class  M_Driver extends CI_Model {
		...
		public function selectTransaksi($id){
				$query = $this->db->query("SELECT * from transaksi inner join user on transaksi.id_user = user.id inner join harga on harga.id_harga = transaksi.paket where id_driver = $id order by startTime");
            			return $query->result_array();
	
		}

		public function selectPerjalanan($id){
				$query = $this->db->query("SELECT * from transaksi inner join rating on transaksi.rating_id = rating.id_rating inner join user on transaksi.id_user = user.id where id_driver = $id order by startTime");
            			return $query->result_array();
